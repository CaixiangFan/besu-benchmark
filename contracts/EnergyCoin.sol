...
contract EnergyCoin {
   ...
   constructor (string memory name, string memory symbol, uint32 decimals, uint32 totalSupply, uint32 exchangeRate) public {
        _symbol=symbol;
        _name=name;
        _decimals=decimals;
   ...
   function registerProducers(address[] memory addresses, uint32 initialBalance, uint32 productionCapacity) public {
   ...
   function registerConsumers(address[] memory addresses, uint32 initialBalance) public {
        require (msg.sender==authorizedEntity);
        for(uint i=0; i<addresses.length;i++){
    ...
    function buyEnergyCoin() public payable {
        if(msg.value <_exchangeRate)
            revert();
       _balanceOf[msg.sender]=uint32(msg.value/_exchangeRate);
    }
    
    function updateAccountBalances(address[] memory winnersAddresses, int32[] memory netPaymentData) public {
       // require (msg.sender==authorizedEntity); The message is coming from Payment settlement Contract. So no need to Specify this requirement
        //Update accounts balances for all users
         for(uint i=0; i<winnersAddresses.length; i++){
    ...
    function getSupplierProductionCapacity(address _addr) public view returns (uint32) {
...
// This method normally cowrks on uint256. I changed it to uint32
library SafeMath {
