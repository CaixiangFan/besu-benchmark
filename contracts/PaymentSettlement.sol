...
contract PaymentSettlement {
 ...
 enum PaymentSettlementState {NotSettled, Settled}
 enum PaymentSettlementResettingState {NotReset, Reset}
 ...
 PaymentSettlementResettingState paymentSettlementResettingState;
   ...
   constructor (address _energyCoinAddress) public {
        ec= EnergyCoin(_energyCoinAddress);
   ...
   function poolMarketSupplierDispatchData( address[] memory _poolMarketSupplierWinners, uint32[] memory _poolMarketSupplierEnergyDispatch, uint32 _poolMarketClearingPrice) public {
        ...
        paymentSettlementResettingState=PaymentSettlementResettingState.NotReset;
...
/*    
    ...
    function smartMeterConsumedEnergy (uint32 consumedAmount) public { // How to prevent losers from sending this function?
        consumedEnergy[msg.sender]=consumedAmount;
        
    }
    
    function smartMeterProducedEnergy (uint32 producedAmount) public{ // How to prevent losers from sending this function?
        producedEnergy[msg.sender]=producedAmount;
        
    }
    
    
   function SettlePayment () public{ //Here we assume that there is only pool market (the others are not implemented yet). Also the settlement is only for one interval
      require (msg.sender==authorizedEntity);
...
/*  for(uint i=0;i<poolMarketConsumerEnergyDispatch.length;i++){
    ...
    for(uint i=0; i<poolMarketConsumerWinners.length;i++){
        address consumerAddress =poolMarketConsumerWinners[i];
        uint32 poolMarketDispachAmount = poolMarketConsumerEnergyDispatch[i];
       // uint32 deviation= consumedEnergy[consumerAddress]-poolMarketDispachAmount;
        uint32 deviation=0; //for now, i assume the deviation is 0 as there is no consumption and production data
        int32 netPayment= int32(poolMarketDispachAmount*poolMarketClearingPrice + deviation*balancingMarketClearingPrice);
        netPaymentDataArray.push(netPayment);
        allWinnersAddresses.push(consumerAddress);
    }
    
   for(uint i=0; i<poolMarketSupplierWinners.length;i++){
        address supplierAddress =poolMarketSupplierWinners[i];
        uint32 poolMarketDispachAmount = poolMarketSupplierEnergyDispatch[i];
       // uint32 deviation= poolMarketDispachAmount-producedEnergy[supplierAddress];
        uint32 deviation=0; //for now, i assume the deviation is 0 as there is no consumption and production data
        int32 netPayment= int32(poolMarketDispachAmount*poolMarketClearingPrice - deviation*balancingMarketClearingPrice);
        netPaymentDataArray.push(netPayment);
    ...
    ec.updateAccountBalances(allWinnersAddresses, netPaymentDataArray);
    }
   
   function getPoolMarketConsumerWinners () public view returns (address [] memory){
       return poolMarketConsumerWinners;
   }
   function getPoolMarketSupplierWinners () public view returns (address [] memory){
       return poolMarketSupplierWinners;
   }
    
    function getPoolMarketSupplierEnergyDispatch () public view returns (uint32 [] memory){
       return poolMarketSupplierEnergyDispatch;
   }
     function getPoolMarketConsumerEnergyDispatch () public view returns (uint32 [] memory){
     ...
     function getAllNetPayment () public view returns (int32 [] memory){
       ...
       function getPaymentSettlementState () public view returns (PaymentSettlementState _paymentSettlementState) {
        return paymentSettlementState;
    }
    
       function getPaymentSettlementResettingState () public view returns (PaymentSettlementResettingState  _paymentSettlementResettingState) {
