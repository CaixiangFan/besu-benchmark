...
contract PoolMarket {
 ...
 //RealTimeMarket realTimeMarket;
  ...
  constructor(address paymentSettlementAddress, address energyCoinContractAddress) public {
        authorizedEntity=msg.sender;
        ps=PaymentSettlement(paymentSettlementAddress);
        ec= EnergyCoin(energyCoinContractAddress);
  }
    ...
    modifier isValidBid (uint32 amount, uint32 priceRate, address addr){
    require(ec.getBalance(addr)>=amount *priceRate);//This also checks whether the user is registered or not because the balance would not be > zero if the users is not registered
    require (minAllowedPrice <= priceRate && priceRate <= maxAllowedPrice);
      _;
    }

   modifier isValidOffer (uint32 amount, uint32 priceRate, address addr) {
     require(ec.getSupplierProductionCapacity(addr)>=amount); //This also checks whether the user is registered or not because the capacity would be zero if the users is not registered
     require (minAllowedPrice <= priceRate && priceRate <= maxAllowedPrice);
        _;
    }
  ...
  function submitEnergyOffer (uint32 amount, uint32 priceRate) public isValidOffer(amount, priceRate, msg.sender){ // Currently this accepts offer only for one interval
  ...
  function submitEnergyBid (uint32 amount, uint32 priceRate) public isValidBid(amount,priceRate, msg.sender){  // Currently this accepts bid only for one interval
      require (biddingState==BiddingState.Open);
  ...
  function calculateMarkerClearingPrice () public { // We assume that the market clearnce is only for one interval
   require (msg.sender==authorizedEntity);
  ...
  for (uint32 i=0; i<suppliers.length;i++){
    ...
    function getAllConsumerDispatchedEnergy () public view returns (uint32 [] memory){
         return consumerEnergyDispatchToPaymentSettlemt;
    }
        
    function getAllSupplierDispatchedEnergy () public view returns (uint32 [] memory){
   ...
   function getBiddingState () public view returns (BiddingState _biddingState) {
        return biddingState;
    }
    
    function getMarketClearanceState () public view returns (MarketClearanceState _marketClearanceState) {
        return marketClearanceState;
    }
    
       function getMarketResettingState () public view returns (MarketResettingState  _marketResettingState) {
...
function sendDataToPaymentSettlement () public {
    ps.poolMarketSupplierDispatchData(supplierWinners,supplierEnergyDispatchToPaymentSettlemt,clearingPrice);
    ps.poolMarketConsumersDispatchData(consumerWinners,consumerEnergyDispatchToPaymentSettlemt,clearingPrice);
}

function SendDataAndResetPoolMarket() public{ // The data has to be sent to Real time contract before it is deleted either in this function or in market clearance
...
/*contract PaymentSettlement {
    function poolMarketSupplierDispatchData( address[] memory _poolMarketSupplierWinners, uint32[] memory _poolMarketSupplierEnergyDispatch, uint32 _poolMarketClearingPrice) public {
 
    }
    
  function poolMarketConsumersDispatchData( address[] memory _poolMarketConsumerWinners, uint32[] memory _poolMarketConsumerEnergyDispatch, uint32 _poolMarketClearingPrice) public {
