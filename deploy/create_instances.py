...
def deploy(network_size, flavor_name, watchdog_address):
    ...
    with open('configuration-script.sh') as f:
        ...
        new_command = post_creation_command[:-3] + \
            str(network_size) + post_creation_command[-2:]
    user_data = base64.b64encode(new_command.encode("utf-8")).decode('utf-8')

    for id in range(network_size):
        instance_name = 'besu-'+str(id+1)
        create_instance(conn=conn, instance_name=instance_name,
                        user_data=user_data, flavor_name=flavor_name)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print('Baseline confiuggurations will be used: N={}, flavor={}'.format(
            network_size, flavor_name))
    else:
        network_size = int(sys.argv[1])
        if len(sys.argv) > 2:
            flavor_name = sys.argv[2]
        print('Default values have been overwritten: N={}, flavor={}'.format(
            network_size, flavor_name))
